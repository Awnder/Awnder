## Hello! I'm Andrew! 👋

I'm a software engineer proficient in Python and learning React/Next.js through building full-stack applications! I'm currently a senior undergraduate computer science major graduating in May 2025!

## My Skills
- **Languages:** JavaScript, Python, C#
- **Frameworks:** React, Next.js, LangChain
- **Tools & Platforms:** Git, GCP, AWS, Vercel
- **Databases:** Supabase, Firebase

## Projects
### In Progress!
1) An open source contribution to [Puter OS](https://github.com/HeyPuter/puter)! Issue [here](https://github.com/HeyPuter/puter/issues/1130).

### Some other ones!
1) I recently completed a data analysis internship where I went outside my role and built a RAG chatbot to onboard other interns! You can read about it [here](https://medium.com/@andrew.shiroma/from-template-to-langchain-rag-enabled-chatbot-4d9b564b60b4).
2) [Roomventory](https://www.roomventory.site/) - your group inventory app with AI suggestions so you never forget a thing!
3) [Lightcard](https://ai-flashcards-pi.vercel.app/) - light your path to success with AI-powered flashcards
4) [Popreel](https://popreel-eight.vercel.app): a TikTok clone created because TikTok was going to get banned lol

---
### 📫 Contact me:
1) follow my tech journey on [LinkedIn](www.linkedin.com/in/andrewshiroma)
2) see my [website](www.andrewshiroma.dev)
3) check out my technical blog on [Medium](https://medium.com/@andrew.shiroma)

🌱 Currently brainstorming software ideas!

## GitHub Stats

![Awnder's GitHub stats](https://github-readme-stats.vercel.app/api?username=Awnder&show_icons=true&theme=radical)

## Fun Fact

I make mead in my free time! It's like wine but instead of grapes it's made with honey! 

---

Thanks for visiting my profile! Have a great day! 😊
